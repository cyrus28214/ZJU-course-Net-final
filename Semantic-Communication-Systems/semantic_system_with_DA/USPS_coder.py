...
for lambda_var in range(1):
    ...
    for rate in range(10):
        ...
        channel = max(np.sqrt(16 * (1 - compression_rate) / 2), 1)
        channel = int(channel)
        ...
        class classifier_svhn(nn.Module):
            def __init__(self, out_ch=16):
                ...
                self.cla1 = nn.Conv2d(3, out_ch, kernel_size=4, stride=1, padding=0)
                self.pool = nn.MaxPool2d(2, stride=2, return_indices=True)  # 14*14
                self.cla2 = nn.Conv2d(out_ch, out_ch, kernel_size=4, stride=1, padding=0)  # 10*10
                self.pool = nn.MaxPool2d(2, stride=2, return_indices=True)  # 5*5
                self.cla3 = nn.Conv2d(out_ch, 10, kernel_size=4, stride=1, padding=0)  # 1*1
                self.relu = nn.ReLU()
        ...
        class coding(nn.Module):
            def __init__(self, out_ch=16):
                ...
                # channel = 2
                self.conv1 = nn.Conv2d(1, out_ch, kernel_size=channel, stride=1, padding=0)
                ...
                self.tconv5 = nn.ConvTranspose2d(out_ch, 1, kernel_size=channel, stride=1, padding=0)
        ...
        # classifier = classifier_svhn().to(device)  # 加载已有的分类器（不需要单独再训练了），但是还得训练编译码器
        ...
        def criterion(x_in, y_in, raw_in):
            ...
            out_tmp2 = nn.MSELoss()
            x_in_reshape = x_in.view(64,-1)
            z_in = classifier(x_in_reshape)
        ...
        for e in range(epoch_len):
            ...
            for im, label in train_data:
                ...
                out_reshape = out.view(64, -1)
                out_svhn = classifier(out_reshape)
            ...
            for im, label in test_data:
                ...
                # classifier.eval()
                out_svhn = classifier(out_reshape)
                im_reshape = im.view(64,-1)
                out_real = classifier(im_reshape)
